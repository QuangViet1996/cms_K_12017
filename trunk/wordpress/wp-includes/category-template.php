...
function walk_category_tree() {
	...
	/*foreach($args[0] as $category){
		if(count($categories) < 3 && $category->parent == 0){
			$categories[] = $category;
			$sub_cates = makeCategories($args[0],$category->term_id);
			
			$categories = array_merge($categories,$sub_cates);
    ...
    foreach($categories as $parent){
    	$idex = 0;
		foreach($args[0] as $category){ 
			if($idex < 3 && $parent->term_id == $category->parent){
				$idex++; 
				$sub_cates[] = $category;
			}
		}
...
function makeCategories($categories = [],$parent){
	$cates = [];

	foreach($categories as $category){
		if(count($categories) < 3 && $category->parent == $parent){
			$cates[] = $category;
			$sub_cates = makeCategories($categories,$category->term_id); 
